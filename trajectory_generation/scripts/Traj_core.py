...
def joint_traj_gen(theta_start,theta_end,t,T,scale):
    """Generate trajectory from valid angle answers, set the robot to move
       5 seconds, one angle change per second
    """
    if scale == 3:
...
def back2home_traj(theta_end):
    """Generate trejactory from current pose to home pose, allow robot to move within 5 sec
    """
    joint_traj = joint_traj_gen(np.array([0,0,0,0]),theta_end,5,5,5)
    theta_b2h = np.zeros((6,4))
    
    theta_b2h[0] = joint_traj[5]
    theta_b2h[1] = joint_traj[4]
    theta_b2h[2] = joint_traj[3]
    theta_b2h[3] = joint_traj[2]
    theta_b2h[4] = joint_traj[1]
    theta_b2h[5] = np.array([0,0,0,0])
